...
<script>
                ...
                set(newVal) { // 改变值的时候 触发set
                    ...
                    if (value !== newVal) {
                        ...
                        watcherTpl._directives.forEach((item) => { // 遍历订阅池
                            item.update();
                            // 遍历所有订阅的地方(v-model+v-bind+`两个大括号`) 触发this._compile()中发布的订阅Watcher 更新视图
                        });
                    }
                }
            })
    ...
    myVue.prototype._compile = function (el) {
        ...
        for (var i = 0, len = nodes.length; i < len; i++) { // 遍历dom节点
            ...
            var reg = /\{\{\s*([^}]+\S)\s*\}\}/g, txt = node.textContent;   // 正则匹配`两个大括号`
            if (reg.test(txt)) {
                node.textContent = txt.replace(reg, (matched, placeholder) => {
                    // matched匹配的文本节点包括`两个大括号`, placeholder 是`两个大括号`中间的属性名
